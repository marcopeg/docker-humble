...
MYSQL_DUMP_FILE_NAME_FORMAT="%s___%p___%d"
...
TARGET_FILE_NAME=${2:-$MYSQL_DUMP_FILE_NAME_FORMAT}
TARGET_DATE=$(date $MYSQL_DUMP_DATE_FORMAT)
TARGET_FILE_NAME="${TARGET_FILE_NAME/\%s/$MYSQL_HOST}"
TARGET_FILE_NAME="${TARGET_FILE_NAME/\%p/$MYSQL_DB}"
TARGET_FILE_NAME="${TARGET_FILE_NAME/\%d/$TARGET_DATE}"


# Compose target file
MYSQL_DUMP_TARGET="$MYSQL_DUMP_ROOT/$TARGET_FILE_NAME"
