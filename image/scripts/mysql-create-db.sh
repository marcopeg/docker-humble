...
MYSQL_DB_CHARSET=${2:-"utf8"}
MYSQL_DB_FORCE_CREATE=${MYSQL_DB_FORCE_CREATE:-"no"}

# Accept "--force" as last option
for LAST_P; do true; done
[ "--force" == "$LAST_P" ] && MYSQL_DB_FORCE_CREATE="yes"
[ "--force" == "$MYSQL_DB_CHARSET" ] && MYSQL_DB_CHARSET="utf8"
...
echo "charset:   $MYSQL_DB_CHARSET"
echo "force:     $MYSQL_DB_FORCE_CREATE"
echo ""
echo "(sleeping 5 secs, you can abort with Ctrl+c)"
sleep 5

# Drop existing database
if [ "yes" == "$MYSQL_DB_FORCE_CREATE" ]; then
    mysql -h $MYSQL_HOST -u$MYSQL_USER -p$MYSQL_PASSWORD -e "DROP DATABASE IF EXISTS ${MYSQL_DB};"
fi

# Create new database
mysql -h $MYSQL_HOST -u$MYSQL_USER -p$MYSQL_PASSWORD -e "CREATE DATABASE ${MYSQL_DB} /*\!40100 DEFAULT CHARACTER SET $MYSQL_DB_CHARSET */;"
